# -*- coding: utf-8 -*-
# code generated by Prisma. DO NOT EDIT.
# pyright: reportUnusedImport=false
# fmt: off

# global imports for type checking
from builtins import bool as _bool
from builtins import int as _int
from builtins import float as _float
from builtins import str as _str
import sys
import decimal
import datetime
from typing import (
    TYPE_CHECKING,
    Optional,
    Iterable,
    Iterator,
    Sequence,
    Callable,
    ClassVar,
    NoReturn,
    TypeVar,
    Generic,
    Mapping,
    Tuple,
    Union,
    List,
    Dict,
    Type,
    Any,
    Set,
    overload,
    cast,
)
from typing_extensions import TypedDict, Literal


LiteralString = str
# -- template models.py.jinja --
from pydantic import BaseModel

from . import fields, actions
from ._types import FuncType
from ._builder import serialize_base64
from ._compat import PYDANTIC_V2, ConfigDict

if TYPE_CHECKING:
    from .client import Prisma


_PrismaModelT = TypeVar('_PrismaModelT', bound='_PrismaModel')


class _PrismaModel(BaseModel):
    if PYDANTIC_V2:
        model_config: ClassVar[ConfigDict] = ConfigDict(
            use_enum_values=True,
            arbitrary_types_allowed=True,
            populate_by_name=True,
        )
    elif not TYPE_CHECKING:
        from ._compat import BaseConfig

        class Config(BaseConfig):
            use_enum_values: bool = True
            arbitrary_types_allowed: bool = True
            allow_population_by_field_name: bool = True
            json_encoders: Dict[Any, FuncType] = {
                fields.Base64: serialize_base64,
            }

    # TODO: ensure this is required by subclasses
    __prisma_model__: ClassVar[str]


class BaseUser(_PrismaModel):
    __prisma_model__: ClassVar[Literal['User']] = 'User'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.UserActions[_PrismaModelT]':
        from .client import get_client

        return actions.UserActions[_PrismaModelT](client or get_client(), cls)


class BaseUserProfile(_PrismaModel):
    __prisma_model__: ClassVar[Literal['UserProfile']] = 'UserProfile'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.UserProfileActions[_PrismaModelT]':
        from .client import get_client

        return actions.UserProfileActions[_PrismaModelT](client or get_client(), cls)


class BasePost(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Post']] = 'Post'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.PostActions[_PrismaModelT]':
        from .client import get_client

        return actions.PostActions[_PrismaModelT](client or get_client(), cls)


class BaseLike(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Like']] = 'Like'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.LikeActions[_PrismaModelT]':
        from .client import get_client

        return actions.LikeActions[_PrismaModelT](client or get_client(), cls)


class BaseComment(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Comment']] = 'Comment'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.CommentActions[_PrismaModelT]':
        from .client import get_client

        return actions.CommentActions[_PrismaModelT](client or get_client(), cls)


class BaseSkill(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Skill']] = 'Skill'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.SkillActions[_PrismaModelT]':
        from .client import get_client

        return actions.SkillActions[_PrismaModelT](client or get_client(), cls)


class BaseTargetSkill(_PrismaModel):
    __prisma_model__: ClassVar[Literal['TargetSkill']] = 'TargetSkill'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.TargetSkillActions[_PrismaModelT]':
        from .client import get_client

        return actions.TargetSkillActions[_PrismaModelT](client or get_client(), cls)


class BaseConversation(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Conversation']] = 'Conversation'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.ConversationActions[_PrismaModelT]':
        from .client import get_client

        return actions.ConversationActions[_PrismaModelT](client or get_client(), cls)


class BaseMessage(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Message']] = 'Message'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.MessageActions[_PrismaModelT]':
        from .client import get_client

        return actions.MessageActions[_PrismaModelT](client or get_client(), cls)


class BaseExchange(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Exchange']] = 'Exchange'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.ExchangeActions[_PrismaModelT]':
        from .client import get_client

        return actions.ExchangeActions[_PrismaModelT](client or get_client(), cls)


class BaseExchangeSkill(_PrismaModel):
    __prisma_model__: ClassVar[Literal['ExchangeSkill']] = 'ExchangeSkill'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.ExchangeSkillActions[_PrismaModelT]':
        from .client import get_client

        return actions.ExchangeSkillActions[_PrismaModelT](client or get_client(), cls)


class BaseReview(_PrismaModel):
    __prisma_model__: ClassVar[Literal['Review']] = 'Review'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.ReviewActions[_PrismaModelT]':
        from .client import get_client

        return actions.ReviewActions[_PrismaModelT](client or get_client(), cls)


class BaseSearchHistory(_PrismaModel):
    __prisma_model__: ClassVar[Literal['SearchHistory']] = 'SearchHistory'  # pyright: ignore[reportIncompatibleVariableOverride]

    @classmethod
    def prisma(cls: Type[_PrismaModelT], client: Optional['Prisma'] = None) -> 'actions.SearchHistoryActions[_PrismaModelT]':
        from .client import get_client

        return actions.SearchHistoryActions[_PrismaModelT](client or get_client(), cls)


